...
public sealed partial class SingleMarkingPicker : BoxContainer
{
    ...
    public void PopulateList(string filter)
    {
        ...
        {
            if (marking.SponsorOnly && SponsorSimpleManager.GetTier() < 3)
            {
                Logger.Error($"deleted marking from selection: {marking.Name}");
                continue;
            }
    ...
    private void SelectMarking(ItemList.ItemListSelectedEventArgs args)
    {
        ...
        if (proto.SponsorOnly && SponsorSimpleManager.GetTier() < 3) //LP edit
            return;
