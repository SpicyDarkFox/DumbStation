...
reagent-effect-guidebook-missing =
    { $chance ->
        [1] Вызывает
       *[other] вызывает
    } неизвестный эффект, так как описание для него ещё не написано
reagent-effect-guidebook-change-glimmer-reaction-effect =
    { $chance ->
        [1] Изменяет
       *[other] изменяет
    } уровень глимера на { $count } ед.
...
reagent-effect-guidebook-chem-restorereroll-psionic =
    ...
    } способность получать пользу от реагентов, открывающих разум
reagent-effect-guidebook-add-moodlet =
    Изменяет настроение на { $amount }
    { $timeout ->
        [0] навсегда
       *[other] на { $timeout } сек.
    }
reagent-effect-guidebook-remove-moodlet = Удаляет эффект настроения «{ $name }».
reagent-effect-guidebook-purge-moodlets = Удаляет все активные непостоянные эффекты настроения.
