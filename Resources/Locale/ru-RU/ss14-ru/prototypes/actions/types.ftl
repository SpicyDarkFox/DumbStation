...
ent-ToggleNightVision = Переключить ночное зрение
    .desc = Включает или выключает режим ночного зрения.
ent-ToggleThermalVision = Переключить термальное зрение
    .desc = Включает или выключает режим тепловизора.
ent-PulseThermalVision = Импульс термального зрения
    .desc = Временно активирует тепловизор.
ent-ActionSelfExtinguish = Потушить себя
    .desc = Позволяет потушить пламя на себе, если вы горите.
