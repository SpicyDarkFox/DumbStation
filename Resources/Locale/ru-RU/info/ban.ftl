...
# Ban string
server-ban-string = { $admin } установил серверный бан ({ $severity } уровень тяжести), который истекает { $expires }, для [{ $name }, { $ip }, { $hwid }] по причине: { $reason }ban-panel-erase = Стереть сообщения в чате и игрока из раунда
