...
public sealed class CharacterRequirementsSystem : EntitySystem
{
    ...
    public bool CheckRequirementValid(CharacterRequirement requirement, JobPrototype job,
        ...
        out string? reason, int depth = 0, int sponsorTier = 0, string uuid = "") //LP edit
    {
        ...
        return
            !requirement.IsValid(job, profile, playTimes, whitelisted, prototype,
                ...
                out reason, depth, null, sponsorTier, uuid)   //LP edit
    ...
    public bool CheckRequirementsValid(List<CharacterRequirement> requirements, EntityUid characterUid, IPrototype prototype, out List<string> reasons, int depth = 0, bool whitelisted = false, int sponsorTier = 0, string uuid = "") //LP edit
    {
        ...
        return CheckRequirementsValid(requirements, jobPrototype, stationSpawningProfile, trackerTimes, whitelisted, prototype, _entManager, _protomanager, _configurationManager, out reasons, depth, mind, sponsorTier, uuid);//LP edit
    }

    public bool CheckRequirementsValid(List<CharacterRequirement> requirements, JobPrototype job,
        HumanoidCharacterProfile profile, IReadOnlyDictionary<string, TimeSpan> playTimes, bool whitelisted, IPrototype prototype,
        IEntityManager entityManager, IPrototypeManager prototypeManager, IConfigurationManager configManager,
        out List<string> reasons, int depth = 0, MindComponent? mind = null, int sponsorTier = 0, string uuid = "") //LP edit
    {
        reasons = new List<string>();
        var valid = true;
        ...
        foreach (var requirement in requirements)
        {
            ...
            if (!requirement.IsValid(job, profile, playTimes, whitelisted, prototype,
                entityManager, prototypeManager, configManager,
                out var reason, depth, mind, sponsorTier, uuid))  //LP edit
            {
                if (valid)
